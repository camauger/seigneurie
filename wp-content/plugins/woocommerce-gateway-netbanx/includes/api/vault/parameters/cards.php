 ...
 */
class Cards extends Parameters_Abstract {
	...
	public function create_from_single_use_token_parameters( $token, $profile_id ) {
		...
		wc_paysafe_add_debug_log( 'Cards: Create from single-use token parameters: ' . print_r( $params, true ) );
	...
	public function update_from_single_use_token_parameters( $profile_id, $card_id, $single_use_token ) {
		...
		wc_paysafe_add_debug_log( 'Cards: Update from single-use token parameters: ' . print_r( $params, true ) );
		
		return $params;
	}
	
	/**
	 * @param \WC_Payment_Token|\WC_Payment_Token_Paysafe_CC $wc_token
	 *
	 * @return array
	 */
	public function delete_method_parameters( $wc_token ) {
		...
		wc_paysafe_add_debug_log( 'Cards: Delete token parameters: ' . print_r( $params, true ) );
	 ...
	 * @return array
	 */
	public function set_default_source_parameters( $wc_token ) {
		return apply_filters( 'wc_paysafe_set_default_source_parameters', array(
			'id'                   => $wc_token->get_source_id(),
			'profileID'            => $wc_token->get_profile_id(),
			'cardExpiry'           => array(
				'month' => $wc_token->get_expiry_month(),
				'year'  => $wc_token->get_expiry_year()
			),
			'holderName'           => $this->get_fields()->get_billing_full_name(),
			'billingAddressId'     => $wc_token->get_billing_address_id(),
